  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setUsuarios([
          { id: 1, nome: "Gabriel Silva", cargo: "Administrador" },
          { id: 2, nome: "Ana Costa", cargo: "Técnico" },
          { id: 3, nome: "Lucas Souza", cargo: "Técnico" },
          { id: 4, nome: "Carla Dias", cargo: "Técnico" },
        ]);

        setProjetos([
          { id: 1, nome: "Torno CNC 01", serie: "HNFDAF323243", status: "Crítico" },
          { id: 2, nome: "Fresadora X2", serie: "JDU8239JDSA", status: "Atenção" },
          { id: 3, nome: "Prensa Hidráulica", serie: "KKI92039432", status: "Saudável" },
          { id: 4, nome: "Compressor Industrial", serie: "PLM0293048", status: "Crítico" },
        ]);
          ...
          <div className="users-list">
            {usuarios.map((user) => (
              <div key={user.id} className="user-item">
                ...
                <Typography variant="caption" className="user-name">{user.nome}</Typography>
                <Typography variant="caption" className="user-role">{user.cargo}</Typography>
              </div>
            ))}
            ...
            {projetos.map((proj) => (
              <div key={proj.id} className="project-row">
                ...
                <div className="project-info">
                  <Typography variant="body2" fontWeight="bold">{proj.nome}</Typography>
                  <Typography variant="caption" color="textSecondary">N° Série: {proj.serie}</Typography>
                </div>
                <Chip label={proj.status} size="small" className="status-chip" />
              </div>
            ))}
          </div>
        </div>
